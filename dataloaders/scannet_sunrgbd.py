...
class ScanNetDataset_sunrgbd(object):
    def __init__(self, data_path):
        ...
        class_names = open(os.path.join('./datasets/ScanNet', 'meta', 'scannet_classnames.txt')).readlines()
        ...
        model_class_names = ['bathtub', 'bed', 'bookshelf', 'chair', 'cabinet', 'curtain', 'desk', 'door', 'counter',
                              'sink', 'sofa', 'table', 'picture', 'toilet', 'refridgerator','window']
        self.novel_classes = ['bed', 'chair', 'sofa', 'table', 'curtain', 'toilet', 'desk', 'door']
        self.test_classes =  [self.type2class[i] for i in self.novel_classes]
        # self.fold_0 = ['bathtub', 'bed', 'bookshelf', 'cabinet', 'chair','counter', 'curtain', 'desk', 'door']
        # self.fold_1 = ['picture', 'refridgerator', 'sink', 'sofa', 'table', 'toilet', 'window']
        ...
        self.train_classes = [self.type2class[c] for c in model_class_names if c not in self.novel_classes]
...
if __name__ == '__main__':
    ...
    dataset = ScanNetDataset_sunrgbd(args.data_path)