...
def sampled_obj_pointclouds(num_point, pc_attribs, pc_augm, pc_augm_config, support_files):
    ptclouds = []
    for support_file in support_files:
        data = np.load(support_file)
        N = data.shape[0]
...
def sample_pointcloud(data_path, num_point, pc_attribs, pc_augm, pc_augm_config, scan_name,
    ...
    rgb = data[:, 3:6]
    labels = data[:,6].astype(np.int64)
    #superpoint = data[:,8].astype(np.int64)
...
class MyDataset(Dataset):
    def __init__(self, data_path, dataset_name, cvfold=0, num_episode=50000, n_way=3, k_shot=5, n_queries=1,
        ...
        if dataset_name == 's3dis':
            ...
            self.data_name = 'S3DIS'
            self.dataset_name = dataset_name
            class_names = open(os.path.join('/home/xiaojiwei/workspace/3Dseg/attMPTI/datasets/S3DIS', 'meta','s3dis_classnames.txt')).readlines()
            self.semantic_label_names = [class_name.strip() for class_name in class_names]
        elif dataset_name == 's3dis_modelnet':
            from dataloaders.s3dis_modelnet import S3DISDataset_ModelNet
            self.dataset = S3DISDataset_ModelNet(data_path)
            self.data_name = 'S3DIS'
            self.dataset_name = dataset_name
            class_names = open(os.path.join('/home/xiaojiwei/workspace/3Dseg/attMPTI/datasets/S3DIS', 'meta','s3dis_classnames.txt')).readlines()
            self.semantic_label_names = [class_name.strip() for class_name in class_names]
        elif dataset_name == 's3dis_sunrgbd':
            from dataloaders.s3dis_sunrgbd import S3DISDataset_sunrgbd
            self.dataset = S3DISDataset_sunrgbd(data_path)
            self.data_name = 'S3DIS'
            self.dataset_name = dataset_name
            class_names = open(os.path.join('/home/xiaojiwei/workspace/3Dseg/attMPTI/datasets/S3DIS', 'meta','s3dis_classnames.txt')).readlines()
            self.semantic_label_names = [class_name.strip() for class_name in class_names]
            ...
            self.data_name = 'ScanNet'
            self.dataset_name = dataset_name
            class_names = open(os.path.join('/home/xiaojiwei/workspace/3Dseg/attMPTI/datasets/ScanNet', 'meta','scannet_classnames.txt')).readlines()
            self.semantic_label_names = [class_name.strip() for class_name in class_names]
        elif dataset_name == 'scannet_modelnet':
            from dataloaders.scannet_modelnet import ScanNetDataset_ModelNet
            self.dataset = ScanNetDataset_ModelNet(data_path)
            self.data_name = 'ScanNet'
            self.dataset_name = dataset_name
            class_names = open(os.path.join('/home/xiaojiwei/workspace/3Dseg/attMPTI/datasets/ScanNet', 'meta','scannet_classnames.txt')).readlines()
            self.semantic_label_names = [class_name.strip() for class_name in class_names]
        elif dataset_name == 'scannet_sunrgbd':
            from dataloaders.scannet_sunrgbd import ScanNetDataset_sunrgbd
            self.dataset = ScanNetDataset_sunrgbd(data_path)
            self.data_name = 'ScanNet'
            self.dataset_name = dataset_name
            class_names = open(os.path.join('/home/xiaojiwei/workspace/3Dseg/attMPTI/datasets/ScanNet', 'meta','scannet_classnames.txt')).readlines()
            self.semantic_label_names = [class_name.strip() for class_name in class_names]
        ...
        elif mode == 'test' or mode =='mptieval' or mode =='genmptieval' or mode == 'valid' or mode == 'protoeval':
    ...
    def __getitem__(self, index, n_way_classes=None):
        ...
        support_ptclouds, query_ptclouds, query_labels = self.generate_obj_one_episode(sampled_classes)
        if self.mode == 'test' or self.mode == 'mptieval' or self.mode == 'valid' or self.mode == 'train' or self.mode == 'protoeval':
            return support_ptclouds.astype(np.float32), \
                query_ptclouds.astype(np.float32), \
                query_labels.astype(np.int64), \
                sampled_classes.astype(np.int32)
        elif self.mode == 'genmptieval':
            return support_ptclouds.astype(np.float32), \
                   query_ptclouds.astype(np.float32), \
                   query_labels.astype(np.int64), \
                   np.concatenate((sampled_classes, np.array(self.dataset.train_classes)), 0).astype(np.int32)


    def generate_obj_one_episode(self, sampled_classes):
        support_ptclouds = []
        query_ptclouds = []
        query_labels = []
        ...
        for sampled_class in sampled_classes:
            ...
            if len(block_list) != 0:
                all_scannames = [x for x in all_scannames if x not in block_list]
            query_scannames = np.random.choice(all_scannames, self.n_queries, replace=False)
            block_list.extend(query_scannames)

            if self.mode == 'train' and self.data_name == 'S3DIS':
                obj_file = glob.glob(os.path.join('/home/xiaojiwei/workspace/3Dseg/attMPTI/datasets',self.data_name, 'objects', self.semantic_label_names[sampled_class], '*.npy'))
            elif self.mode == 'train' and self.dataset_name == 'scannet_modelnet':
                obj_file = glob.glob(os.path.join('/home/xiaojiwei/workspace/3Dseg/attMPTI/datasets',self.data_name, 'modelnet_objects', self.semantic_label_names[sampled_class], 'train', '*.npy'))
            elif self.mode == 'train' and self.dataset_name == 'scannet_sunrgbd':
                obj_file = glob.glob(os.path.join('/home/xiaojiwei/workspace/3Dseg/attMPTI/datasets',self.data_name, 'sunrgbd', 'pointclouds', self.semantic_label_names[sampled_class], '*.npy'))
            elif (self.mode == 'test' or self.mode == 'valid' or self.mode =='mptieval' or self.mode == 'protoeval') and (self.dataset_name == 'scannet_modelnet' or self.dataset_name == 's3dis_modelnet'):
                obj_file = glob.glob(os.path.join('/home/xiaojiwei/workspace/3Dseg/attMPTI/datasets',self.data_name, 'modelnet_objects', self.semantic_label_names[sampled_class], 'train', '*.npy'))
            elif (self.mode == 'test' or self.mode == 'valid' or self.mode =='mptieval' or self.mode == 'protoeval') and (self.dataset_name == 'scannet_sunrgbd' or self.dataset_name == 's3dis_sunrgbd'):
                obj_file = glob.glob(os.path.join('/home/xiaojiwei/workspace/3Dseg/attMPTI/datasets',self.data_name, 'sunrgbd', 'pointclouds', self.semantic_label_names[sampled_class], '*.npy'))
            else:
                obj_file = glob.glob(os.path.join('/home/xiaojiwei/workspace/3Dseg/attMPTI/datasets',self.data_name, 'objects', self.semantic_label_names[sampled_class], '*.npy'))
            #print(self.data_name, semantic_label_names[sampled_class])
            # print(obj_file)
            support_files = np.random.choice(obj_file, self.k_shot, replace=False)


            if self.mode == 'test' or self.mode =='mptieval' or self.mode == 'valid' or self.mode == 'train' or self.mode == 'protoeval': #label 0 - n_way
                query_ptclouds_one_way, query_labels_one_way = sample_K_pointclouds(self.data_path, self.num_point,
                                                                                ...
                                                                                sampled_classes,
                                                                                is_support=False)
            elif self.mode == 'genmptieval': #label 0 - n_way+n_train_class
                gentest_classes = np.concatenate((sampled_classes,np.array(self.dataset.train_classes)),0)
                query_ptclouds_one_way, query_labels_one_way = sample_K_pointclouds(self.data_path, self.num_point,
                                                                                    self.pc_attribs, self.pc_augm,
                                                                                    self.pc_augm_config,
                                                                                    query_scannames,
            ...
            support_ptclouds_one_way = sampled_obj_pointclouds(self.num_point, self.pc_attribs, self.pc_augm, self.pc_augm_config, support_files)
        ...
        return support_ptclouds, query_ptclouds, query_labels
...
def batch_obj_task_collate(batch):
    batch_support_ptclouds, batch_query_ptclouds, batch_query_labels, batch_sampled_classes = batch[0]

    data = [torch.from_numpy(batch_support_ptclouds).transpose(2,3),
...
class MyTestDataset(Dataset):
    def __init__(self, data_path, dataset_name, cvfold=0, num_episode_per_comb=100, n_way=3, k_shot=5, n_queries=1,
        ...
        dataset = MyDataset(data_path, dataset_name, cvfold=cvfold, n_way=n_way, k_shot=k_shot, n_queries=n_queries,
                            mode=mode, num_point=num_point, pc_attribs=pc_attribs, pc_augm=False)
        self.classes = dataset.classes

        if mode == 'valid':
            test_data_path = os.path.join(data_path, 'S_%d_N_%d_K_%d_episodes_%d_pts_%d_%s' % (
                                                    cvfold, n_way, k_shot, num_episode_per_comb, num_point, dataset_name))
        elif mode == 'mptieval' or mode == 'test' or mode == 'protoeval':
            test_data_path = os.path.join(data_path, 'S_%d_N_%d_K_%d_test_episodes_%d_pts_%d_%s' % (
                                                    cvfold, n_way, k_shot, num_episode_per_comb, num_point, dataset_name))
        elif mode == 'genmptieval':
            test_data_path = os.path.join(data_path, 'S_%d_N_%d_K_%d_gentest_episodes_%d_pts_%d_%s' % (
                                                    cvfold, n_way, k_shot, num_episode_per_comb, num_point, dataset_name))
            self.genclasses = dataset.dataset.train_classes+dataset.dataset.test_classes
        else:
...
def write_episode(out_filename, data):
    support_ptclouds,  query_ptclouds, query_labels, sampled_classes = data
...
def read_episode(file_name):
    ...
    return support_ptclouds, query_ptclouds, query_labels, sampled_classes
