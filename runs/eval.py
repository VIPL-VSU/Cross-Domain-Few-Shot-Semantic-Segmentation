...
from dataloaders.loader import MyTestDataset, batch_test_task_collate, batch_obj_task_collate
...
def eval(args):
    ...
    if args.phase == 'protoeval' or args.phase == 'genmptieval':
        if args.use_zero:
    ...
    TEST_DATASET = MyTestDataset(args.data_path, args.dataset, cvfold=args.cvfold,
                                 ...
                                 num_point=args.pc_npts, pc_attribs=args.pc_attribs, mode=args.phase)
    TEST_CLASSES = list(TEST_DATASET.classes)
    print(TEST_CLASSES)
    TEST_LOADER = DataLoader(TEST_DATASET, batch_size=1, shuffle=False, collate_fn=batch_obj_task_collate)

    if args.phase == 'genmptieval':
        GENTEST_CLASSES = list(TEST_DATASET.genclasses)
        test_loss, mean_IoU = test_few_shot(TEST_LOADER, learner, logger, GENTEST_CLASSES, args.use_zero)
    else:
