...
if __name__ == '__main__':
    ...
    parser.add_argument('--phase', type=str, default='graphtrain', choices=['pretrain', 'finetune',
                                                                            ...
                                                                            'mptitrain', 'mptieval', 'genmptieval', 'visualize'])
    ...
    parser.add_argument('--obj', action='store_true', help='use obj encoder')
    parser.add_argument('--use_o2s', action='store_true', help='use o2s module')
    parser.add_argument('--use_multiple_discriminative_prototypes', action='store_true', help='use contrast module')
    ...
    if args.phase == 'mptitrain':
        ...
        args.log_dir = args.save_path + 'log_proto_%s_S%d_N%d_K%d_Att%d_obj_%d_tr_%d_o2s_%d_mdp_%d_mah_%d' % (args.dataset, args.cvfold,
                                                                             args.n_way, args.k_shot,
                                                                             args.use_attention, args.obj, args.use_transformer,args.use_o2s, args.use_supervise_prototype, args.use_mahalanobis)
        from runs.proto_train import train
        train(args)
    elif args.phase == 'protoeval' or args.phase == 'mptieval' or args.phase == 'genmptieval':
        ...
        from runs.visualize import visualization
        visualization(args)
    else:
